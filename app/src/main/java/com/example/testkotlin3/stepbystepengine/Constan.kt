...
import com.example.testkotlincode.expression.AddExp
import com.example.testkotlincode.expression.FormalExpression
import com.example.testkotlincode.expression.MonomialExp
...
class Constan(_latexExpression: String) : MathType(_latexExpression) {
    ...
    public fun removeM(m: Double): String {
    ...
    public fun drawGraph(graph: GraphView, m: Double) {
        ...
        for (i in 1..500) {
            ...
            y = listMonoExptoAddExp(nonParaLatexToFormalExpression(removeM(m), 'x')).evalute(x)
            if (x == 0.0) {
    ...
    override public fun solution(): List<String> {
        ...
        listStrResult.add(b01)
        if (haveC(this.latexExpression, 'm') == true) {
            var b02 = "-Nhập vào giá trị của m : ";
            listStrResult.add(b02)
            //*** Sau đó giá trị m sẽ lấy từ view, truyền vào hàm remove M, t được một hàm số không chứa tham số
        }
        var latexNonM = removeM(m)
        var b03 = "-Với m=" + m + " ta có hàm số sau " + latexNonM
        listStrResult.add(b03)
